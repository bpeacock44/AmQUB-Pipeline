...
[Part 2: Demultiplexing and Trim Stat Generation](https://github.com/bpeacock44/AmQUB-Pipeline/blob/main/02_mbio_tutorial.md#part-2-demultiplexing-and-trim-stat-generation) 
Demultiplexes the data and estimates how many reads you’ll retain after trimming and quality filtering.

[Part 3: Taxonomic Unit (TU) Selection](https://github.com/bpeacock44/AmQUB-Pipeline/blob/main/02_mbio_tutorial.md#part-3-taxonomic-unit-tu-selection)  
Processes the data to define TUs (e.g., OTUs or ASVs) and generates the initial count table.

[Part 4: Taxonomic Assignment](https://github.com/bpeacock44/AmQUB-Pipeline/blob/main/02_mbio_tutorial.md#part-4-taxonomic-assignment)
Assigns taxonomy to TUs using BLAST against the NCBI nt database and, optionally, a Qiime2 classifier.

[Part 5: Final Processing](https://github.com/bpeacock44/AmQUB-Pipeline/blob/main/02_mbio_tutorial.md#part-5-final-processing)  
...
👉 Examples of how the full pipeline can be run are provided [here.](https://github.com/bpeacock44/AmQUB-Pipeline/blob/main/03_sample_runs.sh) 
...
Templates are provided in the [templates folder](https://github.com/bpeacock44/AmQUB-Pipeline/tree/main/param_templates). You can edit them in a text editor or in Excel (but save as CSV).
...
### ➡️ Parameters for the read filtering section of the pipeline:
...
### ➡️ Parameters related to the command that finds unique reads in your filtered dataset.
...
### ➡️ Parameters related to TU seed sequence picking.
...
### ➡️ Parameters for using Strategy 2.
...
### ➡️ Parameter for using Strategy 3.
...
### ➡️ Parameters for making the TU table:
...
#### 2. **Number of Threads Available (`-t`) [Required]**  
   ...
   - See [01_setup_tutorial.md](https://github.com/bpeacock44/AmQUB-Pipeline/blob/main/01_setup_tutorial.md) for information on requesting resources for this part.
...
#### 7. **Classifier (`-c`) [Optional, default is no classifier step]** 
   - Path to the classifier you want to use. See [Getting Classifiers for Qiime2](https://github.com/bpeacock44/AmQUB-Pipeline/blob/main/02_mbio_tutorial.md#getting-classifiers-for-qiime2) below.
...
2. **Optionally, removes TUs in one of two ways**
    - A file can be provided for one or multiple strategies indicating certain TUs that should be removed prior to further processing. (See flags `-1`, `-2`, or `-3`.)
    - A file can be provided containing taxonomic strings to be removed. (See flag `-e`.) For example, we can include a file containing the string "Viridiplantae" and all OTUs with "Viridiplantae" in the taxonomic lineage will be removed.
...
Exclude Taxa File #(OPTIONAL ROW)
...
#### 5. **Exclude Taxa File (`-e`) [Optional, default is no removal]**
   - This is a text file. Each line is a string representing any undesireable taxa. Any TUs with a taxonomic lineage containing these strings will be removed from all output tables.

#### 6. **Universal Assay (`-u`) [Optional, default is no splitting]**
...
#### 7. **Classifier Assignments Primary (`-c`) [Optional, default is to use the BLAST assignments]** 
...
#### 8. **Detailed Informational TU Table (`-d`) [Optional, default is to not generate this table]** 
   - This enables the creation of the Detailed Informational TU Table. (See details above under [📚 What does this part do?](https://github.com/bpeacock44/AmQUB-Pipeline/blob/main/02_mbio_tutorial.md#-what-does-this-part-do-4))

#### 9. **Mixed Top 10 Analysis (`-m`) [Optional, default is to not do this analysis]** 
   - This causes the mixed top 10 analysis to be conducted. (See details above under [📚 What does this part do?](https://github.com/bpeacock44/AmQUB-Pipeline/blob/main/02_mbio_tutorial.md#-what-does-this-part-do-4)

#### 10. **Strategy flags (`--nostrategy1`, `--strategy2`, `--strategy3`)** 
