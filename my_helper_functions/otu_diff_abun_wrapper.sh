...
# Usage: ./otu_diff_abun_wrapper.sh <column> <factor 1> <factor 2>
...
biom2txt otu_table_02_add_taxa.nc.biom otu_table_02_add_taxa.nc.txt
file1="otu_table_02_add_taxa.nc.txt"
...
for element in "${elements[@]}"; do
    if [[ -n $element && $element != "#OTU ID" && $element != "taxonomy" ]]; then
...
if awk -F'\t' -v dc="$diff_col" 'NR==1{for(i=1;i<=NF;i++) if($i==dc) {dc_flag=1}; if(dc_flag) exit 0; exit 1}' merged_map.txt; then
    ...
    if awk -F'\t' -v dc="$tissue_col" -v va="$var_a" -v vb="$var_b" 'BEGIN{found_a=0; found_b=0} {if($dc==va) found_a=1; if($dc==vb) found_b=1} END{if(found_a && found_b) exit 0; exit 1}' merged_map.txt; then
        ...
        differential_abundance.py -i otu_table_02_add_taxa.nc.biom -o diff_otus.txt -m merged_map.txt -a DESeq2_nbinom -c "$diff_col" -x "$var_a" -y "$var_b" -d

        # Run Rscript otu_diff.R
        echo "Adding average normalized counts to the output."
        Rscript otu_diff.R diff_otus.txt merged_map.txt otu_table_02_add_taxa_norm.nc.txt "$diff_col" "$var_a" "$var_b"
    else
...
echo
echo " - -- --- ---- ---- --- -- -"
echo "differential_abundance_results.txt and diff_otus_diagnostic_plots.pdf have been successfully generated using the differential_abundance.py program from Qiime1."
echo " - -- --- ---- ---- --- -- -"